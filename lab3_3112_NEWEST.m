...
for i=1:3
    ...
    % plot
    figure()
    plot(xtemp,vtemp,"LineWidth",2);
    hold on
    grid on
    plottitle = strcat('2 Element Mode Shape at ' + string(round(freq_2(i),2)) + ' Hz');
    title(plottitle);
...
for i=1:3
    ...
    hold on
    grid on
    plottitle = strcat('4 Element Mode Shape at ' + string(round(freq_4(i),2)) + ' Hz');
    title(plottitle);
    xlabel("Length along bar [in]")
    ylabel("Displacement")
    grid on
